...
export function OperationsCenterPage() {
  ...
  return <div style={{ display: 'grid', gap: 18 }}>
    <section className="opsHero"><div className="opsHeroTop"><div><div className="opsTitle"><span className="opsTitleIcon">OC</span> Operations Center</div><div className="opsSub">Unified audit, diagnostics, readiness, exports, delivery logs, and validation tools.</div></div><div className="opsToolbar"><button className="btn btnGhost" disabled={busy} onClick={() => post('health', '/api/v1/admin/health/run-checks')}>Run checks</button><button className="btn btnPrimary" onClick={() => setTab('actions')}>Admin actions</button></div></div><div className="opsKpis"><div className="opsKpi"><div className="opsKpiLabel">System</div><div className="opsKpiValue">{healthQ.data?.ok ? 'Good' : 'Check'}</div><div className="opsKpiHint">DB {healthQ.data?.checks?.db?.latencyMs ?? '-'}ms</div></div><div className="opsKpi"><div className="opsKpiLabel">Readiness</div><div className="opsKpiValue">{requiredReady ? 'Ready' : 'Needs work'}</div><div className="opsKpiHint">Required checklist</div></div><div className="opsKpi"><div className="opsKpiLabel">Services</div><div className="opsKpiValue">{Object.keys(services).length ? `${configured}/${Object.keys(services).length}` : '-'}</div><div className="opsKpiHint">Configured</div></div><div className="opsKpi"><div className="opsKpiLabel">Failed delivery</div><div className="opsKpiValue">{failed}</div><div className="opsKpiHint">Recent rows</div></div></div></section>
