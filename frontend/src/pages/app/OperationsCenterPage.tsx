...
import { ProjectMigrationHealthPanel } from '../../components/admin/ProjectMigrationHealthPanel'
    ...
    <div className="pageHeaderCard">
      <div className="pageHeaderCardInner" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
        <div>
          ...
          <div className="pageHeaderCardSub">Unified audit, diagnostics, readiness, exports, delivery logs, and validation tools.</div>
          <div className="pageHeaderCardMeta" style={{ marginTop: 10, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            <span className="pageHeaderCardTag">System: {healthQ.data?.ok ? '✓ Good' : '— Check'}</span>
            <span className="pageHeaderCardTag">Readiness: {requiredReady ? '✓ Ready' : '⚠ Needs work'}</span>
            <span className="pageHeaderCardTag">Services: {Object.keys(services).length ? `${configured}/${Object.keys(services).length}` : '—'} configured</span>
            {failed > 0 && <span className="pageHeaderCardTag" style={{ color: '#ef4444', background: 'rgba(239,68,68,0.10)', borderColor: 'rgba(239,68,68,0.22)' }}>⚠ {failed} failed deliveries</span>}
          </div>
        </div>
        <div style={{ display: 'flex', gap: 8, flexShrink: 0, flexWrap: 'wrap' }}>
          <button className="btn btnGhost" disabled={busy} onClick={() => post('health', '/api/v1/admin/health/run-checks')}>Run checks</button>
          <button className="btn btnPrimary" onClick={() => setTab('actions')}>Admin actions</button>
        </div>
      </div>
    </div>
