...
type Project = { id: string; name: string; description?: string | null; color?: string | null; status?: 'active' | 'paused' | 'completed'; task_count?: number }
...
export function ProjectsPage() {
  ...
  const [viewMode, setViewMode] = useState<ProjectViewMode>(() => {
    const saved = typeof window !== 'undefined' ? window.localStorage.getItem('tf_projects_view') : null
    return saved === 'list' || saved === 'grid' ? saved : 'grid'
  })

  function changeViewMode(mode: ProjectViewMode) {
    setViewMode(mode)
    if (typeof window !== 'undefined') window.localStorage.setItem('tf_projects_view', mode)
  }
    ...
    <div style={{ display: 'grid', gap: 18 }}>
      <div className="pageHeaderCard">
        <div className="pageHeaderCardInner">
          ...
          {canCreate && (
            <button type="button" className="btn btnPrimary" onClick={() => setCreateOpen(true)} style={{ display: 'inline-flex', alignItems: 'center', gap: 7 }}>
        ...
        <div style={{ padding: '16px' }}>
          ...
          {!q.isLoading && projects.length === 0 && (
            <div className="emptyStateV3" style={{ padding: '40px 20px' }}>
              ...
              <div className="emptyStateV3Body">{canCreate ? 'Create your first project to start organizing tasks' : 'Projects with tasks assigned to you will appear here.'}</div>
              {canCreate && (
          ...
          {projects.length > 0 && (
            <div className={`projectsCollection ${viewMode === 'list' ? 'projectsCollectionList' : 'projectsCollectionGrid'}`}>
              {projects.map(p => (
                <button
                  key={p.id} type="button" className="projectCardButton"
                ...
                >
                  <div style={{ width: 38, height: 38, borderRadius: 11, flexShrink: 0, background: (p.color || '#e2ab41') + '20', border: `1.5px solid ${(p.color || '#e2ab41')}40`, display: 'grid', placeItems: 'center' }}>
                    <div style={{ width: 14, height: 14, borderRadius: 4, background: p.color || '#e2ab41' }} />
                  </div>
                  <div className="projectCardBody" style={{ minWidth: 0 }}>
      ...
      <Modal
        ...
        footer={<div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', width: '100%' }}><button type="button" className="btn btnGhost" onClick={() => { setCreateOpen(false); setError(null) }}>Cancel</button><button type="submit" form="createProjectForm" className="btn btnPrimary" disabled={m.isPending}>{m.isPending ? 'Creating…' : 'Create Project'}</button></div>}
      >
        ...
        <form id="createProjectForm" onSubmit={onSubmit} style={{ display: 'grid', gap: 14 }}>
          <Input label="Project Name" required placeholder="e.g. Onboarding, Payroll, Sales Ops" value={name} onChange={e => setName(e.target.value)} autoFocus icon={<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"/></svg>} />
          <div className="inputV3Wrap"><label className="selectV3Label">Description <span style={{ color: 'var(--muted)', fontSize: 10, textTransform: 'none', letterSpacing: 0 }}>(optional)</span></label><div className="inputV3Field textareaField" style={{ height: 'auto' }}><textarea className="inputV3Native" value={description} onChange={e => setDescription(e.target.value)} placeholder="Brief description of this project…" style={{ height: 72, paddingTop: 10, paddingBottom: 10, resize: 'vertical' }} /></div></div>
          <div className="inputV3Wrap"><label className="selectV3Label">Accent Color</label><ColorPickerTrigger value={color} onChange={setColor} /></div>
        </form>
      </Modal>

      <ProjectDetailModal projectId={detailId} onClose={() => setDetailId(null)} onOpenTasks={() => { setDetailId(null); navigate('/app/tasks') }} />
    </div>
  )
}
