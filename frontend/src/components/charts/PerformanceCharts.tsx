...
function GlassTooltip({ active, payload, label }: any) {
  ...
  return (
    <div style={{ background: 'var(--bg1,#101522)', border: '1px solid rgba(255,255,255,.16)', borderRadius: 12, padding: '12px 16px', boxShadow: '0 16px 40px rgba(0,0,0,0.35)', fontSize: 12, color: 'var(--text)', minWidth: 140 }}>
...
// Shell uses explicit pixel height + ResponsiveContainer for responsive width
// This fixes both "chart width mismatch" (empty space) and "0px height" issues
...
const AXIS = { fill: 'var(--chart-tick2,#94a3b8)', fontSize: 11, fontWeight: 650 }

function EmptyChart({ icon, title }: { icon: string; title: string }) {
  return (
    <div className="emptyStateV3" style={{ padding: '40px 20px', minHeight: 220, display: 'grid', placeItems: 'center' }}>
...
export function StatusBarChart(props: { byStatus: Record<string, number>; fillHeight?: boolean }) {
  ...
  return (
    <Shell height={height}>
      <BarChart data={data} barSize={36} margin={{ top: 14, right: 12, left: 0, bottom: 36 }}>
...
export function PriorityPieChart(props: { byPriority: Record<string,number>; fillHeight?: boolean }) {
  ...
  const total = data.reduce((s,d) => s+d.value, 0)
  return (
    <div style={{ display:'grid', gridTemplateColumns:'1fr minmax(130px,0.85fr)', gap:16, alignItems:'center', minHeight:260 }}>
        ...
        {data.map(d => (
          <div key={d.name} style={{ display:'flex', alignItems:'center', gap:10 }}>
            ...
            <div style={{ flex:1, minWidth:0 }}>
              ...
              <div style={{ height:4, borderRadius:2, background:'rgba(148,163,184,0.16)', marginTop:4, overflow:'hidden' }}>
                <div style={{ height:'100%', width:`${(d.value/total)*100}%`, background:d.fill, borderRadius:2 }} />
              </div>
              <div style={{ fontSize:10, color:'var(--muted)', marginTop:2, fontWeight:700 }}>{Math.round(d.value/total*100)}%</div>
            </div>
          </div>
        ))}
      </div>
  ...
  return (
    <Shell height={height}>
      <AreaChart data={props.points} margin={{ top:14, right:18, left:0, bottom:8 }}>
        ...
        <Area type="monotone" dataKey="due" name="Created" stroke="#6366f1" strokeWidth={3} fill="#6366f1" fillOpacity={0.12} dot={false} activeDot={{ r:6, strokeWidth:3, stroke:'var(--bg)', fill:'#6366f1' }} />
        <Area type="monotone" dataKey="completed" name="Completed" stroke="#22c55e" strokeWidth={3} fill="#22c55e" fillOpacity={0.12} dot={false} activeDot={{ r:6, strokeWidth:3, stroke:'var(--bg)', fill:'#22c55e' }} />
        <Area type="monotone" dataKey="overdue" name="Overdue" stroke="#ef4444" strokeWidth={3} fill="#ef4444" fillOpacity={0.10} dot={false} activeDot={{ r:6, strokeWidth:3, stroke:'var(--bg)', fill:'#ef4444' }} />
      </AreaChart>
    </Shell>
  )
}
  ...
  return (
    <div style={{ display:'grid', gap:14, minHeight:300 }}>
      <div style={{ position:'relative' }}>
        <Shell height={210}>
          <RadialBarChart cx="50%" cy="50%" innerRadius="65%" outerRadius="95%" barSize={14} startAngle={90} endAngle={-270} data={[{ name:'Balanced', value:balancedPct, fill:'#22c55e' }]}>
...
export function AssigneeScoreChart(props: {
  ...
  return (
    <Shell height={height}>
      <BarChart data={data} layout="vertical" barCategoryGap="34%" margin={{ top:8, right:80, left:4, bottom:0 }}>
