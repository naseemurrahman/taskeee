...
const H = 280
const AXIS = { fill: 'var(--chart-tick2,#94a3b8)', fontSize: 11, fontWeight: 600 }
const gridStroke = 'rgba(148,163,184,0.18)'

function GlassTooltip({ active, payload, label }: any) {
  if (!active || !payload?.length) return null
  return (
    <div style={{ background:'var(--bg1,#101522)', border:'1px solid rgba(255,255,255,.16)', borderRadius:12, padding:'12px 16px', boxShadow:'0 16px 40px rgba(0,0,0,0.35)', fontSize:12, color:'var(--text)' }}>
...
// Shell: explicit height div + responsive width via ResponsiveContainer
// This is the correct fix: the shell's height is pixel-set (so no 0px issue),
// and the width is responsive (fills any card width, no empty space)
        ...
        <div key={p.name}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:6 }}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:10, flexShrink:0 }}>
              ...
              <span style={{ fontSize:13, fontWeight:900, color:p.color, minWidth:42, textAlign:'right' }}>{p.pct}%</span>
            </div>
          </div>
          <div style={{ height:10, borderRadius:5, background:'rgba(148,163,184,0.16)', overflow:'hidden', position:'relative' }}>
