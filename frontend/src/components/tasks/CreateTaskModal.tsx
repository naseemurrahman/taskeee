...
export function CreateTaskModal(props: { open: boolean; onClose: () => void; defaultProjectId?: string | null }) {
  ...
  const capacityGuidance = useMemo(() => {
    return filteredAssignees
      .map(user => {
        const workload = workloadById.get(user.id)
        const openTasks = Number(workload?.open_tasks || 0)
        const totalTasks = Number(workload?.total_tasks || 0)
        const tone = workloadTone(openTasks, averageOpenTasks)
        return {
          id: user.id,
      ...
      .slice(0, 5)
  }, [averageOpenTasks, filteredAssignees, workloadById])
  ...
  const m = useMutation({
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ['modal-workload'] })
      handleClose()
    },
    onError: (err) => {
  ...
  return (
    ...
    >
      ...
      <form onSubmit={onSubmit} style={{ display: 'grid', gap: 14 }}>
          ...
          <Select
            ...
            options={[
              ...
              ...filteredAssignees.map(u => {
                const workload = workloadById.get(u.id)
                const openTasks = Number(workload?.open_tasks || 0)
                return {
                  value: u.id,
                  label: u.full_name || u.name || u.email,
                  description: [u.department, u.role, `${openTasks} open`].filter(Boolean).join(' · '),
                }
              }),
            ]}
        ...
        {canAssign && (
          <div className="capacityGuideCard">
            <div className="capacityGuideHeader">
              <div>
                <div className="capacityGuideTitle">Capacity guidance</div>
                <div className="capacityGuideSub">Suggested by current open workload. Manager keeps final assignment control.</div>
              </div>
              <span className="capacityGuideAvg">Avg {averageOpenTasks.toFixed(1)} open</span>
            </div>
            {workloadQ.isLoading ? (
              <div className="capacityGuideEmpty">Loading workload…</div>
            ) : capacityGuidance.length === 0 ? (
              <div className="capacityGuideEmpty">No workload data available yet.</div>
            ) : (
              <div className="capacityGuideList">
                {capacityGuidance.map(candidate => {
                  const selected = candidate.id === assignedTo
                  return (
                    <button key={candidate.id} type="button" className={`capacityGuideItem ${selected ? 'capacityGuideItemSelected' : ''}`} onClick={() => setAssignedTo(candidate.id)}>
                      <div style={{ minWidth: 0 }}>
                        <div className="capacityGuideName">{candidate.name}</div>
                        <div className="capacityGuideMeta">{candidate.department || candidate.role || 'Team member'} · {candidate.totalTasks} total tasks</div>
                      </div>
                      <div className="capacityGuideStats">
                        <span className="capacityGuideLoad">{candidate.openTasks} open</span>
                        <span className="capacityGuideTone" style={{ color: candidate.tone.color, background: candidate.tone.bg }}>{candidate.tone.label}</span>
                      </div>
                    </button>
                  )
                })}
            ...
            {selectedGuidance && (
              <div className="capacityGuideSelected">
                Selected: <strong>{selectedGuidance.name}</strong> · {selectedGuidance.openTasks} open tasks · {selectedGuidance.tone.label}
              </div>
            )}
          </div>
        )}
