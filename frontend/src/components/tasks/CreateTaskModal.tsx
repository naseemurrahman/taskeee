...
export function CreateTaskModal(props: { open: boolean; onClose: () => void; defaultProjectId?: string | null; initialDueDate?: string | null }) {
  ...
  function onSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    if (!projectId) { setError('Project is required. Please choose a project.'); return }
    if (!priority) { setError('Priority is required. Please choose a priority.'); return }
    if (!assignedTo) { setError('Assign To is required. Please select an employee.'); return }
    if (!dueDate) { setError('Due date is required. Please select a date.'); return }
  ...
  return (
    <Modal title="Create Task" subtitle="All fields are required except description." open={props.open} onClose={handleClose} icon={<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>}>
      ...
      <form aria-label="Create task form" onSubmit={onSubmit} style={{ display: 'grid', gap: 14 }}>
        <div className="requiredFieldHint">Required: title, project, priority, assignee, and due date. Description is optional.</div>
        <Input label="Task Title" required name="title" autoFocus placeholder="e.g. Prepare onboarding plan for new joiner" icon={<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>} />
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          <Select label="Project" required value={projectId} onChange={setProjectId} options={[{ value: '', label: projectsQ.isLoading ? 'Loading projects…' : 'Select project…', disabled: true }, ...projects.map(p => ({ value: p.id, label: p.name, color: p.color || undefined }))]} />
          <Select label="Priority" required value={priority} onChange={setPriority} options={PRIORITIES.map(p => ({ value: p.value, label: p.label, description: p.description, color: p.color }))} />
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: hasDepts ? '1fr 1fr' : '1fr', gap: 12 }}>
          {hasDepts && <div><Select label="Department" value={dept} onChange={v => { setDept(v); setAssignedTo('') }} options={[{ value: '', label: 'All departments' }, ...departments.map(d => ({ value: d, label: d }))]} searchable={departments.length > 5} />{dept && <div style={{ fontSize: 11, color: 'var(--brand)', fontWeight: 700, marginTop: 3, paddingLeft: 2 }}>↓ {filteredAssignees.length} employee{filteredAssignees.length !== 1 ? 's' : ''} in {dept}</div>}</div>}
          <Select label="Assign To" required value={assignedTo} onChange={setAssignedTo} searchable options={[{ value: '', label: usersQ.isLoading ? 'Loading employees…' : allUsers.length === 0 ? 'No team members — add employees first' : `Select employee… (${filteredAssignees.length})`, disabled: true }, ...filteredAssignees.map(u => { const workload = workloadById.get(u.id); const openTasks = Number(workload?.open_tasks || 0); return { value: u.id, label: u.full_name || u.name || u.email, description: [u.department, u.role, `${openTasks} open`].filter(Boolean).join(' · ') } })]} />
        </div>
        {canAssign && <div className="capacityGuideCard"><div className="capacityGuideHeader"><div><div className="capacityGuideTitle">Capacity guidance</div><div className="capacityGuideSub">Suggested by current open workload. Manager keeps final assignment control.</div></div><span className="capacityGuideAvg">Avg {averageOpenTasks.toFixed(1)} open</span></div>{workloadQ.isLoading ? <div className="capacityGuideEmpty">Loading workload…</div> : capacityGuidance.length === 0 ? <div className="capacityGuideEmpty">No workload data available yet.</div> : <div className="capacityGuideList">{capacityGuidance.map(candidate => { const selected = candidate.id === assignedTo; return <button key={candidate.id} type="button" className={`capacityGuideItem ${selected ? 'capacityGuideItemSelected' : ''}`} onClick={() => setAssignedTo(candidate.id)}><div style={{ minWidth: 0 }}><div className="capacityGuideName">{candidate.name}</div><div className="capacityGuideMeta">{candidate.department || candidate.role || 'Team member'} · {candidate.totalTasks} total tasks</div></div><div className="capacityGuideStats"><span className="capacityGuideLoad">{candidate.openTasks} open</span><span className="capacityGuideTone" style={{ color: candidate.tone.color, background: candidate.tone.bg }}>{candidate.tone.label}</span></div></button> })}</div>}{selectedGuidance && <div className="capacityGuideSelected">Selected: <strong>{selectedGuidance.name}</strong> · {selectedGuidance.openTasks} open tasks · {selectedGuidance.tone.label}</div>}</div>}
        <Input label="Due Date" required name="dueDate" type="date" defaultValue={props.initialDueDate || ''} icon={<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>} />
        <div><label style={{ display: 'block', fontSize: 12, fontWeight: 800, color: 'var(--text2)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 6 }}>Description <span style={{ textTransform: 'none', letterSpacing: 0, fontWeight: 500, fontSize: 11, color: 'var(--muted)' }}>(optional)</span></label><div className="textareaWrap"><textarea name="description" className="textareaInner" style={{ minHeight: 72 }} placeholder="Context, acceptance criteria, links…" /></div></div>
        <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', paddingTop: 4 }}><button type="button" className="btn btnGhost" onClick={handleClose} disabled={m.isPending}>Cancel</button><button type="submit" className="btn btnPrimary" disabled={m.isPending || !canAssign}>{m.isPending ? 'Creating…' : 'Create Task'}</button></div>
      </form>
    </Modal>
  )
}
