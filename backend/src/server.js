...
process.on('unhandledRejection', (reason) => { const msg = reason instanceof Error ? reason.stack || reason.message : String(reason); logger.error('Unhandled promise rejection: ' + msg); });
...
const taskCanonicalGuardsRoutes = require('./routes/taskCanonicalGuards');
...
const taskWorkflowRoutes = require('./routes/taskWorkflow');
...
const canonicalProjectsRoutes = require('./routes/canonicalProjects');
const projectCompletionCompatRoutes = require('./routes/projectCompletionCompat');
const projectLifecycleCompatRoutes = require('./routes/projectLifecycleCompat');
const canonicalProjectLifecycleRoutes = require('./routes/canonicalProjectLifecycle');
const hrisRoutes = require('./routes/hris');
const lifecycleTransactionsRoutes = require('./routes/lifecycleTransactions');
const statusGovernanceRoutes = require('./routes/statusGovernance');
const reassignmentGovernanceRoutes = require('./routes/reassignmentGovernance');
...
const adminProjectMigrationRepairRoutes = require('./routes/adminProjectMigrationRepair');
...
const defaultOrigins = ['http://localhost:5173','http://127.0.0.1:5173','http://localhost:5174','http://127.0.0.1:5174','http://localhost:8080','http://127.0.0.1:8080'];
const allowedOrigins = (process.env.CLIENT_ORIGIN || defaultOrigins.join(',')).split(',').map(o => o.trim()).filter(Boolean);
console.log('Allowed CORS origins:', allowedOrigins);

const io = new Server(server, { cors: { origin: '*', credentials: true, methods: ['GET', 'POST', 'PUT', 'DELETE', 'OPTIONS'], allowedHeaders: ['Content-Type', 'Authorization', 'Cache-Control', 'Pragma'] } });
if (!process.env.CLIENT_ORIGIN) console.warn('⚠️ CLIENT_ORIGIN not set, WebSocket may not work properly in production');
app.set('io', io);

io.use((socket, next) => {
  const token = socket.handshake.auth?.token;
  if (!token) return next(new Error('Authentication required'));
  try { const d = jwt.verify(token, process.env.JWT_SECRET); socket.userId = d.userId; socket.orgId = d.orgId; next(); }
  catch (err) { if (err?.name !== 'TokenExpiredError') logger.warn(`WebSocket authentication error: ${err?.message || 'Invalid token'}`); next(new Error('Invalid token')); }
});
io.on('connection', socket => {
  if (socket.userId) socket.join(`user:${socket.userId}`);
  socket.on('join:org', ({ orgId }) => { if (orgId && String(orgId) === String(socket.orgId)) { socket.join(`org:${orgId}`); socket.join(`org_${orgId}`); } });
  socket.on('join:user', ({ userId }) => { if (userId && String(userId) === String(socket.userId)) socket.join(`user:${userId}`); });
  socket.on('join_task', ({ taskId }) => { if (taskId) socket.join(`task:${taskId}`); });
  socket.on('notify:user', ({ targetUserId, notification }) => { if (socket.orgId) socket.to(`user:${targetUserId}`).emit('notification', notification); });
});

app.use(requestContext);
app.use(helmet({ contentSecurityPolicy: { directives: { defaultSrc: ["'self'"], scriptSrc: ["'self'"], styleSrc: ["'self'"], imgSrc: ["'self'",'data:','blob:'], connectSrc: ["'self'"], frameSrc: ["'none'"], objectSrc: ["'none'"] } }, hsts: { maxAge: 31536000, includeSubDomains: true, preload: true } }));
app.use(securityHeaders);
app.use(compression());
app.use(cors({ origin: true, credentials: true, methods: ['GET','POST','PATCH','DELETE','OPTIONS'], allowedHeaders: ['Content-Type','Authorization','Cache-Control','Pragma','X-Requested-With','X-Request-ID'] }));
...
app.use(rateLimit({ windowMs: 15 * 60 * 1000, max: 300, standardHeaders: true, legacyHeaders: false, message: { error: 'Too many requests — try again later.' } }));

app.get('/api/v1/insights/overview', (_req, res) => res.status(200).json({ generated_at: new Date().toISOString(), summary: { total_tasks: 0, completed_tasks: 0, pending_tasks: 0, overdue_tasks: 0, completion_rate: 0, overdue_rate: 0, average_ai_confidence: 0, average_open_tasks: 0 }, insights: [] }));
...
app.use('/api/v1/recurring-tasks', recurringTasksRoutes);
app.use('/api/v1/tasks', taskWorkflowRoutes);
app.use('/api/v1/tasks', taskBulkActionsRoutes);
app.use('/api/v1/tasks', taskRoutes);
app.use('/api/v1/photos', photoRoutes);
app.use('/api/v1/reports', reportRoutes);
app.use('/api/v1/notifications', notificationRateLimiter, notifRoutes);
...
app.use('/api/v1/projects', projectCompletionCompatRoutes);
app.use('/api/v1/projects', projectLifecycleCompatRoutes);
...
app.use('/api/v1/admin', adminProjectMigrationRepairRoutes);
...
async function buildHealthPayload() {
  ...
  try { await query('SELECT 1'); checks.db = 'ok'; } catch { checks.db = 'error'; }
  try { const { getRedis } = require('./utils/redis'); const rc = getRedis(); if (rc) { await rc.ping(); checks.redis = 'ok'; } else { checks.redis = 'unconfigured'; } } catch { checks.redis = 'error'; }
  return { status: checks.db === 'ok' ? 'ok' : 'starting', uptime: Math.floor(process.uptime()), latencyMs: Date.now() - start, checks, timestamp: new Date().toISOString() };
}

app.get('/health', async (_req, res) => res.status(200).json(await buildHealthPayload()));
app.get('/api/v1/health', async (_req, res) => res.status(200).json(await buildHealthPayload()));
